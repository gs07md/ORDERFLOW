...
def sync_order_dates():
    ...
    known = _load_known()
    moved = removed = gone = 0

    # An order DELETED in Value-FAS has to disappear from the phone too.
    #
    # This was insert-only in both directions: it added orders and moved
    # them, but an order that stopped existing in Value-FAS sat on the
    # website for ever. On 14-09-2026 the shop deleted orders 10163 and
    # 10164 (A ONE AUTO) in Value-FAS and both were still listed in Bill
    # Maker hours later -- "mai delete mari didhi che toy amm dekhde
    # che". Value-FAS never clears Entry='ORD' even after billing, so an
    # order missing from this list is one that was really deleted.
    #
    # The tripwire matters: `live` holds every order the books have ever
    # carried, so it is always thousands. If it comes back small the
    # read went wrong, and nothing is removed on the strength of it.
    safe_to_prune = len(live) > 50
    for order_no, group in by_no.items():
        ...
        if not cur_live:
            ...
            for r in group:
                if _touched(r):
                    print(f"[keep] Order {order_no} is gone from Value-FAS "
                          f"but somebody has entered qty/rate on it -- left "
                          f"on the website for you to look at.")
                    continue
                try:
                    _sb("DELETE", "orders?id=eq."
                        + urllib.parse.quote(str(r["id"]), safe=""))
                    known.discard(r["id"])
                    gone += 1
                    print(f"[info] Order {order_no} was deleted in Value-FAS "
                          f"-- taken off the website too.")
                except Exception as e:
                    print(f"[warn] Could not remove order {order_no}: {e}")
            continue
    ...
    if moved or removed or gone:
        _save_known(known)
        print(f"[info] Date check: {moved} order(s) moved, "
              f"{removed} duplicate(s) removed, "
              f"{gone} deleted in Value-FAS and taken off the website.")
